...
/**
 * 템플릿이 없을 때 쓰는 내장문 — **템플릿과 같은 절차를 지시해야 한다.**
 *
 * 둘이 어긋나면 설치 형태에 따라 세션이 다르게 움직이는데, 그 차이는 헤드리스라서
 * 아무도 못 본다. templates/bootstrap_prompt.txt 를 고치면 여기도 같이 고친다
 * (회귀 테스트가 두 문서의 절차·종료 코드·무결성 조항 일치를 확인한다).
 */
export const BUILTIN_BOOTSTRAP = [
  ...
  "[3] <AH> run --task <id> 로 검증합니다.",
  "[4] 종료 코드 분기: 0=검증 통과 — git commit 후 [2]로 / 1=검증 실패 — 오류 요약을 읽고 자가 수정 후",
  "    재실행(시도 횟수는 장부가 셉니다) / 2=설정 오류 — 중단·보고 / 3=진행 가능 작업 없음 — 요약 후 종료 /",
  "    4=해당 작업 봉인(blocked) — 남은 작업이 있으면 [2]로 계속, 없으면 요약 후 종료.",
  "검증 무결성 — 단정문 삭제·완화, skip/xfail 추가, 광범위 예외 삼킴, 기대값 하드코딩, 실패를 성공으로",
  "마킹 — 어느 것으로도 통과시키지 마십시오. done 은 오직 run 종료 코드 0 으로만 생깁니다.",
  "사람 경계 — 판단이 막히는 작업은 <AH> set-task --id <id> --status blocked --note \"<사유>\" 로 기록하고",
  "다음 작업으로 넘어갑니다.",
  "작업 속도(품질 기준을 덮지 않는 선에서) — 독립적인 조회는 묶어서 호출하고, 긴 로그는 last_log_file 의",
  "필요한 부분만 읽고, 이미 확인한 사실은 다시 확인하지 않으며, 넓은 탐색은 위임해 결론만 받습니다.",
  "더 할 일이 없으면 진행 요약을 출력하고 조용히 종료하십시오.",
].join("\n");
