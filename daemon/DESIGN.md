...
| 빌드 시간 | 0.7~0.9초 | (없음) | `--compile` 은 선빌드된 런타임에 번들을 덧붙인다 |
| 훅 콜드 스타트 p50 | 71~78ms | — | `hook-prebash` + stdin + 장부 적재 상태 |
| **훅 콜드 스타트 p95 (유휴)** | **75~82ms** | **< 150ms** | **예산 이내** — 훅 위임 경로(로컬 소켓)는 불필요 |
| 훅 콜드 스타트 p95 (부하 중) | 110~181ms | < 150ms | **초과할 수 있다** — 아래 주석 |

> **단일 측정으로 판정하지 않는다.** 위 유휴 수치는 4회 측정(75.3 / 81.3 / 82.3 / 82.2ms)에서
> 일관됐지만, 여러 에이전트가 동시에 도는 부하 상태에서 재면 p95 가 110~181ms 까지 올라
> 예산을 넘긴 관측도 있다. 즉 예산 충족은 **유휴 조건부**다. 상시 초과가 관측되면 그때
> 훅 위임 경로(로컬 소켓)를 도입한다 — 지금은 불필요하다는 판단이 유효하다.
...
### 5.2 훅 경로는 기계 고유값이다 — 저장소를 옮기면 죽는다

훅 명령에는 **설치 시점의 절대 EXE 경로**가 박힌다(`"<EXE>" <op> --repo "${CLAUDE_PROJECT_DIR}"`).
저장소 쪽은 플레이스홀더로 옮겨 다닐 수 있지만 엔진 쪽은 그럴 수 없다 — 훅은 프로젝트 루트가
아니라 현재 작업 디렉토리에서 실행되므로 상대 경로를 쓸 수 없고(§5.1 의 근거와 같다), Claude
Code 는 `~`·`%USERPROFILE%` 을 풀어 주지 않는다. 그래서 절대 경로가 유일한 선택지다.

문제는 `.claude/settings.json` 이 **git 추적 대상이라 저장소를 따라 다닌다**는 것이다. 클론하거나
계정·기계를 옮기면 그 경로는 사라지고 게이트 4종이 통째로 죽는다.

> **실측 (2026-08-11, 이 저장소.)** 훅 4종이 `C:\Users\ruinp\…\autoharness.exe` 를 부르는 채로
> 다른 계정에서 열려 있었다. 커밋 게이트·금지 명령 차단·SHA 동기화·Stop 게이트가 전부 무효였고,
> done 101건 중 21건에 커밋 SHA 가 없는 것이 그 흔적이었다. 그런데 진단은 이를 `inactive`
> (등록됐으나 발화 없음)로만 말해 **"저장소 루트에서 claude 를 실행하십시오"라는 틀린 처방**을
> 냈다 — 없는 파일은 어디서 실행해도 없다.

그래서 두 가지를 나눈다. **경로가 cwd 에 흔들리지 않는가**(`pathIsRooted`)와 **그 파일이 실제로
있는가**(`resolvableEnginePath` + 존재 확인)는 다른 조건이다. 전자만 보던 것이 이 자리를 놓쳤다.

- 진단은 `broken_path` 를 별도 상태로 낸다(`dead_engine_hooks` 에 해당 명령을 싣는다).
  이 상태는 **과거 발화 기록을 이긴다** — 예전에 돌았더라도 지금 없는 파일은 못 부른다.
- 설치·마이그레이션은 죽은 경로를 현재 실행 파일로 다시 쓴다(기존 설정은 백업).
- **확인할 수 없는 형태는 죽었다고 단정하지 않는다**(오탐 금지): PATH 로 푸는 이름
  (`autoharness`), `~`·`%VAR%`, 구분자가 있는 상대 경로(이쪽은 `cwd_dependent_hooks` 소관).

운영 지침: 저장소를 새 기계·새 계정에서 처음 열면 `autoharness install --repo <저장소>` 를 한 번
돌린다. 상태 확인은 `status` 의 `hooks.state` 가 `broken_path` 인지 보면 된다.

### 5.3 부트스트랩 프롬프트 — 두 세대를 모두 안다

데몬이 헤드리스 세션을 띄울 때 주는 지시문이다. 설치본 템플릿
(`~/.claude/skills/autoharness/templates/bootstrap_prompt.txt`)이 있으면 그것을, 없으면 내장
폴백문(`supervisor.ts` 의 `BUILTIN_BOOTSTRAP`)을 쓴다.

**둘은 같은 절차를 지시해야 한다.** 어긋나면 설치 형태에 따라 세션이 다르게 움직이는데,
헤드리스라 그 차이는 아무도 못 본다. 회귀 테스트(`test/bootstrap.test.ts`)가 두 문서의
절차·종료 코드 5갈래·무결성 조항 일치와, 프롬프트가 이름을 댄 서브커맨드가 전부 실제
`MODES` 에 있는지를 확인한다.

> **실측 (2026-08-11.)** 템플릿이 `python scripts/harness_engine.py next` 를 지시하고 있었다.
> v2 설치본에는 그 파일이 없다(§5) — 자동 부활한 세션이 첫 명령부터 실패했고, 오류는 세션
> 로그에만 남아 워치독은 "기동했다"고만 보고했다.

프롬프트는 먼저 v2 EXE 존재를 확인해 있으면 그것을 `--repo` 와 함께 쓰고, 없을 때만 v1 경로를
쓰도록 지시한다. 처리량 지시(독립 조회 묶어 호출, 긴 로그는 부분 읽기, 넓은 탐색은 위임)를
포함하되 **그것이 무결성 조항을 덮지 않음을 명시한다** — 빨리 끝내려고 검증을 건너뛰는 것은
위반이다.
...
5. 데몬이 **OS 스케줄러 없이** 24시간 이상 tick 을 유지하고 `last_tick` 이 계속 갱신됨을 실측.
   ...
   **지속 동작 실측 (2026-08-10)**
   ...
   | 항목 | 방식 | 결과 |
   |---|---|---|
   | 드리프트 | 가속 시계 500 tick(약 5일치) | 간격 편차 0 — 모든 간격이 정확히 15분 |
   | 드리프트(실시간) | 실제 타이머 60 tick × 500ms | 29.5초 경과, 평균 간격 500ms, 누적 드리프트 ~0 |
   | last_tick 갱신 | 실제 타이머 관측 | 59회 갱신 — 한 번 쓰고 멈추지 않는다 |
   | 절전 복귀 | 앞으로 6시간 점프 | tick 폭풍 없음(밀린 24주기를 다음 슬롯 하나로 접음) |
   | 시간 변경 | 뒤로 3시간 점프 | 멈추지 않음(지금 기준으로 재계산) |
   | 타이머 누수 | realSleep 200회 | abort 리스너 add 200 = remove 200 |
   | 메모리 | 가속 시계 2000 tick | 힙 증가 24MiB 미만 / 실시간 60 tick 1.3MiB |
   | 잠금 | 종료 시 | 반납 확인 — 다음 기동이 막히지 않는다 |

   24시간 연속 관측은 사람이 실제 설치 후 확인한다(`autostart-live-verify`) — 여기서는
   가속 시계로 그보다 긴 구간을, 실제 타이머로 연속성을 각각 덮는다.
