...
| 요청 신호 | 모드 |
|---|---|
| "하네스 구축/설치/세팅" · 새 저장소에 자율 주행 시작 · **결과 서술형 다단계 요청**인데 대상 저장소에 장부가 **없을 때** | **init** |
| 헤드리스 부트스트랩 재개 · "이어서 진행" · `CLAUDE_AUTOHARNESS=1` 환경 · **결과 서술형 다단계 요청**인데 장부가 **이미 있을 때**(→ `task_add` 로 새 작업을 적재한 뒤 resume) | **resume** |
...
2. **어느 행에도 확신이 안 서면 스킬을 포기하지 말고** 대상 저장소의
   `.claude/agent_tracker.json` 존재로 판정한다 — 있으면 `task_add` 로 작업을 적재한 뒤
   **resume**, 없으면 **init**. (장부가 있는데 init 을 다시 돌리면 진행 상태가 날아간다.)
3. **대상이 아닌 것**: 단발성 질의·코드 설명·파일 한 곳 수정처럼 검증 루프가 필요 없는 작업.
   이때만 스킬 없이 직접 처리한다.
4. **금지**: 모드가 안 맞는다는 이유로 장부·검증 게이트·훅 없이 **맨손으로 다단계 작업을
   수행하는 것**. 그러면 커밋 게이트·금지 명령 차단·테스트 약화 금지·Stop 게이트·워치독이
   전부 무력화된다 — 스킬을 부른 의미가 사라지는 조용한 실패다. 애매하면 2번 규칙으로 간다.
