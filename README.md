...
- **한눈에**: 프로젝트별 진행률 막대(완료·진행·실패·막힘을 색과 숫자로), 지금 붙잡고 있는
  작업과 경과 시간, 다음 기동까지 남은 시간(백오프 중이면 그쪽을 먼저 알립니다)
- 전체 상태와 프로젝트별 상태·백오프·다음 예정 작업
- 선택한 프로젝트의 장부(작업·상태·시도 횟수·커밋 SHA)와 **실패 원인 첫 줄**
  (전문은 '자세히')
- 막힌 곳 요약, 승인 대기 큐(사람 판단으로 봉인된 작업 재투입), 훅 배선 진단
- **주행 중인 Claude Code 세션의 출력** — 데몬의 판단 로그와 구분해 실시간으로 흐릅니다
  (전체 / 세션 출력만 / 데몬 판단만 으로 필터)
- 제어 버튼: 일시정지·재개·즉시 tick·세션 기동, 작업 상태 전환, 검증 명령·한도 편집

조작:

- 화살표(또는 `j`/`k`)로 프로젝트 이동, `/` 로 콘솔 검색, `g` 로 콘솔 고정, `d` 로 테마
  전환, `?` 로 단축키 도움말. **상태를 바꾸는 동작에는 단축키가 없습니다** — 오타 한 번으로
  세션이 뜨면 안 되기 때문입니다(Tab 으로 닿고 Enter 로 누릅니다).
- 테마는 OS 설정을 따르되 수동 선택이 이깁니다. 선택은 토큰과 같이 이 탭에만 남습니다.
- 연결에 실패하면 **원인을 갈라 알립니다**: 응답 자체가 없으면 "데몬 없음"(주소·기동 확인),
...
`python scripts/run_checks.py` 는 3단계입니다.
...
매 실행마다 집합을 대조합니다 — 계획한 모듈·파일 집합과 실제로 돈 테스트 수가 발견 집합과
어긋나면 통과가 아니라 실패입니다(샤드 누락 = 조용한 커버리지 축소).
...
직렬이 기준선(124초)보다 조금 느린 것은 단위마다 프로세스를 새로 띄우기 때문이며, 그
분해가 병렬을 가능하게 한 대가입니다. 하한은 최장 **불가분** 단위가 정합니다(현재
`session-stream.test.ts` 23초).
